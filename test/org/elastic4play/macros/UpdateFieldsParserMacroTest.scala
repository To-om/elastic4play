...
class UpdateFieldsParserMacroTest extends Specification with TestUtils {
  ...
  "UpdateFieldParser macro" should {
    ...
    "make all fields of complex class updatable" in {
      ...
      fieldsParser.parsers.keys.map(_.toString) must contain(exactly(
        "",
        "name",
        "value",
        "subClasses",
    ...
    "parse complex class" in {
      val fieldsParser = getUpdateFieldsParser[ComplexClassForFieldsParserMacroTest]
      val fields = FObject(
        "subClasses[0].name" → FString("sc1"),
        "subClasses[1].option" → FNull)
      val updates = Map(
        FPath("subClasses[0].name") → UpdateOps.SetAttribute("sc1"),
    ...
    "parse class with annotation" in {
      ...
      val updates = Map(
        ...
        FPath("valueEn") → UpdateOps.SetAttribute(LocaleInt(3)))
      fieldsParser(fields) must_=== Good(updates)
    }

    "parse class with implicit" in {
      implicit val subClassFieldsParser: UpdateFieldsParser[SubClassForFieldsParserMacroTest] = UpdateFieldsParser[SubClassForFieldsParserMacroTest]("SubClassForFieldsParserMacroTest")(
        FPath("option") → FieldsParser[UpdateOps.Type]("Optional Int") {
          case (_, FString(intStr)) ⇒ Or.from(Try(intStr.toInt)) match {
      ...
      val fieldsParser = getUpdateFieldsParser[ComplexClassForFieldsParserMacroTest]
      val fields = FObject(
        "subClasses[0].option" → FString("3"),
        "subClasses[1].option" → FString("invalid => unset attribute"))
      val updates = Map(
