...
abstract class DatabaseState {
  def version: Int
  def source(tableName: String): Source[JsObject, Future[Long]]
...
class MigrationSrv @Inject() (
  ...
  class MigrationTransition(db: DBConfiguration, previousState: DatabaseState, operations: Seq[Operation]) extends DatabaseState {
    ...
    override def getEntity(tableName: String, id: String): Future[JsObject] = {
      previousState.getEntity(tableName, id).flatMap { previousValue ⇒
        operations.foldLeft((_: String) ⇒ Source.single(previousValue).mapMaterializedValue(_ ⇒ Future.successful(1L)))((f, op) ⇒ op(f))(tableName)
          .runWith(Sink.head)
      }
    }
  }
  ...
  case class OriginState(db: DBConfiguration) extends DatabaseState {
    ...
    override def version = db.version
    override def source(tableName: String): Source[JsObject, Future[Long]] =
      currentdbfind.apply(Some("all"), Nil)(indexName ⇒ search(indexName → tableName).query(QueryDSL.any.query))
  ...
  object EmptyState extends DatabaseState {
    ...
    override def source(tableName: String): Source[JsObject, Future[Long]] = Source.empty[JsObject].mapMaterializedValue(_ ⇒ Future.successful(0L))
  ...
  def migrate: Future[Unit] = {
    ...
    if (!dbindex.indexStatus && migrationProcess.isCompleted) {
      ...
      migrationProcess = migrationPath(db)
        .flatMap { mig ⇒ dbindex.initIndex(models).map(_ ⇒ mig) }
...
trait Operation extends Function1[String ⇒ Source[JsObject, Future[Long]], String ⇒ Source[JsObject, Future[Long]]]
object Operation {
  lazy val log = Logger(getClass)

  def apply(o: (String ⇒ Source[JsObject, Future[Long]]) ⇒ String ⇒ Source[JsObject, Future[Long]]) = new Operation {
    def apply(f: (String ⇒ Source[JsObject, Future[Long]])) = o(f)
  }
  def renameEntity(previous: String, next: String): Operation = Operation((f: String ⇒ Source[JsObject, Future[Long]]) ⇒ {
  ...
  def mapEntity(tableFilter: String ⇒ Boolean, transform: JsObject ⇒ JsObject): Operation = Operation((f: String ⇒ Source[JsObject, Future[Long]]) ⇒ {
  ...
  def removeEntity(tableFilter: String ⇒ Boolean, filter: JsObject ⇒ Boolean): Operation = Operation((f: String ⇒ Source[JsObject, Future[Long]]) ⇒ {
  ...
  def renameAttribute(tableFilter: String ⇒ Boolean, newName: String, oldNamePath: Seq[String]): Operation = Operation((f: String ⇒ Source[JsObject, Future[Long]]) ⇒ {
