...
import org.elastic4play.utils.Selector
...
class RecordMacro(val c: whitebox.Context) {
    ...
    def unapply(fTpe: Type): Option[(Type, Type)] =
      fTpe.dealias match {
        case RefinedType(l, _) ⇒
          ...
          l.last match {
            case TypeRef(_, `keyTagSym`, List(k, v1)) if v1 =:= rf ⇒ Some(k → rf)
            case _                                                 ⇒ None
          }
        case _ ⇒ None
      }
  ...
  object Record {
    ...
    def unapply(l: Type): Option[(Type, Type, Type)] = {
      ...
      else l.baseType(hconsTpe.typeSymbol) match {
        case TypeRef(pre, _, List(FieldType(k, v), lTail)) if pre =:= hconsPre ⇒ Some((k, v, lTail))
        case _ ⇒ None
      }
    }
  }
  ...
  object KeyTag {
    def unapply(tTpe: Type): Option[String] = {
      tTpe.dealias match {
        case RefinedType(List(`symbolTpe`, TypeRef(tPre, `tagSym`, List(ConstantType(Constant(name: String))))), _) if tPre =:= tagPre ⇒ Some(name)
        case _ ⇒ None
      }
    }
  }
  ...
  def extractHlistTypes(hl: Type): List[(String, Type)] = {
    ...
    def unfold(l: Type, acc: List[(String, Type)]): List[(String, Type)] = {
      if (l <:< hnilTpe) acc
        ...
        case TypeRef(pre, _, List(FieldType(KeyTag(k), v), lTail)) if pre =:= hconsPre ⇒ unfold(lTail, (k, v) :: acc)
  ...
  def mkSelector[L <: HList, K](implicit lTag: WeakTypeTag[L], kTag: WeakTypeTag[K]): Tree = {
    ...
    unfold(lTpe, kTpe) match {
      case Some((v, i)) ⇒
        q" new org.elastic4play.macros.UnsafeSelector[$lTpe, $kTpe, $v]($i) "
      case _ ⇒
        ...
        q" new org.elastic4play.macros.UnsafeSelector[$lTpe, $kTpe, Nothing](0) "
    }
  }
}