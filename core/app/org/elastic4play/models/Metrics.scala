...
class Metrics private (values: immutable.Map[String, Long]) extends Map[String, Long] with immutable.MapLike[String, Long, Metrics] {
  override def empty = Metrics.empty

  override def get(key: String) = values.get(key)

  override def iterator = values.iterator

  override def +[V >: Long](kv: (String, V)): immutable.Map[String, V] = values + kv

  def +(kv: (String, Long))(implicit d: DummyImplicit): Metrics = new Metrics(values + kv)
...
object Metrics {
  ...
  //      })
  //    }
  implicit val metricsReads: Reads[Metrics] = Reads.mapReads[Long].map(new Metrics(_))
