...
import org.elastic4play.JsonFormat.attributeCheckingErrorWrites
...
class ErrorHandler extends HttpErrorHandler {
  ...
  def toErrorResult(ex: Throwable): Option[(Int, JsValue)] = {
    ex match {
      ...
      case rte: RemoteTransportException ⇒
        rte.getCause match {
          case _: IndexNotFoundException ⇒ Some(510 → JsNull)
  ...
  def onServerError(request: RequestHeader, exception: Throwable): Future[Result] = {
    ...
    exception match {
      case AuthenticationError(message) ⇒ Logger.info(s"${request.method} ${request.uri} returned $status: $message")
      case ex                           ⇒ Logger.info(s"${request.method} ${request.uri} returned $status", ex)
    }
