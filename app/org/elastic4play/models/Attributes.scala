...
case class ObjectAttributeFormat(subAttributes: Seq[Attribute[_]]) extends AttributeFormat[JsObject]("nested") {
  ...
  override def checkJson(subNames: Seq[String], value: JsValue) = fromInputValue(subNames, JsonInputValue(value))
...
object AttributeFormat {
  ...
  def enumFmt[T <: Enumeration](e: T)(implicit tag: ClassTag[T], format: Format[T#Value]) = EnumerationAttributeFormat[T](e)
  val uuidFmt = UUIDAttributeFormat
  val hashFmt = HashAttributeFormat
  val binaryFmt = BinaryAttributeFormat
  def listEnumFmt(enumerationName: String)(dblists: DBLists) = ListEnumeration(enumerationName)(dblists)
...
case class Attribute[T](
  ...
  lazy val isReadonly = options.contains(AttributeOption.readonly)
